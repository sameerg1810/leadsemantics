...
if (userParams) {
  // Decode and parse the user object from the URL query parameter
  const user = JSON.parse(decodeURIComponent(userParams));

  // Store the user object in session storage
  sessionStorage.setItem("user", JSON.stringify(user));

  // Now you can access the user details and token as needed
...
const toggleTableContainer = () => {
  ...
  if (tableContainer.style.display === "none") {
    ...
    showTableButton.style.backgroundColor = "green";
    uploadWindow.style.display = "none";
    initTables();
  } else {
    tableContainer.style.display = "none";
    showTableButton.style.backgroundColor = "rgb(23,162,184)";
...
function displayPopup(title, message) {
  console.log("displayPopup is working now");
...
let tokendata = JSON.parse(sessionStorage.getItem("user_data"));
...
// Display popup function (replace with your actual popup code)
...
async function storeTableData(data) {
  console.log("this is Data you're getting", data);
  ...
  try {
    ...
    const response = await fetch(`/checkTable`, {
      method: "POST",
      headers: {
    ...
    if (result.exists) {
      ...
      displayConfirmationPopup(
        `The table '${tableName}' already exists. Do you want to continue?`,
        storeDataCallback
      );
    } else {
      ...
      await storeDataCallback(); // Call the inner function directly with await
    }
  } catch (error) {
  ...
  async function storeDataCallback() {
    // Logic for storing the data in the database goes here
    ...
    console.log("Storing table data with request body:", requestBody);

    try {
      const response = await fetch(`/storeTableData`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!response.ok) {
        console.error("Error storing table data:", response.statusText);
        displayPopup(
          "ERROR storing the data in the database",
          response.statusText
        );
      } else {
        console.log("Table data stored successfully.");
        const result = await response.json();
        console.log("Table data stored:", result);

        // Introduce a time delay (e.g., 2000 milliseconds = 2 seconds) before displaying the popup
        setTimeout(() => {
          displayPopup("Table has successfully saved to the database");
        }, 2000); // Adjust the delay time as needed (in milliseconds)
      }
    } catch (error) {
      console.error("Error storing table data:", error);
      displayPopup("ERROR storing the data in the database", error);
    }
  }
}
...
function displayTableWindow(data, tableName) {
  ...
  data.forEach((row) => {
    ...
    //delete buttons in cell___________________________________________________
    ...
    deleteRecord.appendChild(deleteIcon);
    ...
    async function saveRowData(tableRow, rowId, tableName) {
      ...
      if (Object.keys(updatedData).length > 0) {
        await updateTableData(Id, tableName, oldData, updatedData, rowId); // Pass oldData
  ...
  // declaring delete button___________________________________________
...
function updateTableData(userId, tableName, oldData, updatedData, rowId) {
  ...
  try {
    ...
    const requestBody = {
      user_id: userId,
      file_name: tableName, // Provide the appropriate user ID
      operation: "Update",
      old_value: oldData || "N/A",
      new_value: updatedData || "N/A",
      rowId: rowId || "N/A",
    };

    // Make the POST request to the server route
    const response = fetch(`/insertNotification`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    displayPopup(`${rowId}: Updated ${tableName}`, updatedData);
    initTables();
    DisplayTableData();
  } catch (error) {
...
function deleteTable(tableName) {
  ...
  fetch(`/tables/${tableName}`, {
    ...
    .then((data) => {
      ...
      // Call the initTables function to update the table list
      initTables();
    })
    .catch((error) => {
...
function updateTableList(data) {
  ...
  deleteTableItem.addEventListener("click", function () {
    ...
    deleteSelectedTables(Id, selectedTableNames);
  });
}

// Function to delete selected tables
function deleteSelectedTables(userId, tableName) {
  console.log("the user_id iam getting in deleteSelectedTable", userId);
  console.log("the tableName iam getting at deleteSelectedTable", tableName);
  ...
  // Send a request to the backend to indicate the desire to delete the selected tables
  fetch(`/insertNotification`, {
    ...
    body: JSON.stringify({
      user_id: userId,
      file_name: tableName,
      operation: "DELETE", // Specify the operation
    }),
  })
    .then((response) => response.json())
    .then((data) => {
      console.log(data.message);
      displayPopup("Delete request was sent successfully", data.message);
...
//   axios
//     .post(`/deleteSelectedTables`, { tableNames: selectedTableNames })
//     .then((response) => {
//       console.log("Tables deleted successfully:", selectedTableNames);
//       // Display the deleted message
//       displayPopup("Selected Tables has been deleted successfully");
//       initTables();
//       const tableBody = document.getElementById("table-body");
//       tableBody.innerHTML = "This Table has been deleted";
//       tableBody.classList.add("deleted-message"); // Add the CSS class to the tableBody element
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    console.log("line 1721 this is jwt we are getting from BE");
...
//       // Prepare HTML content to display all 'notify' records
//       const modalContent = document.createElement("div");
//       modalContent.className = "modal-content";
//       modalContent.innerHTML = `
//         <div class="modal-header">
//           <h5 class="modal-title">Notify Data</h5>
//           <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
//         </div>
//         <div class="modal-body">
//           ${data.map((record) => createRecordHTML(record)).join("")}
//         </div>
...
//       modal.setAttribute("aria-labelledby", "notifyModalLabel");
...
//       const notifyModal = new bootstrap.Modal(modal);
...
//       Swal.fire("Error", "Error fetching 'notify' data", "error");
//     });
// });

// function createRecordHTML(record) {
//   const { note_id, user_id, operation, old_value, new_value, file_name } =
//     record;
//   return `
...
//       <p><strong>File Name:</strong> ${file_name || "filename"}</p>
//       <button type="button" class="btn btn-success" onclick="grantPermission(${note_id})">Grant</button>
//       <button type="button" class="btn btn-danger" onclick="denyPermission(${note_id})">Deny</button>
//     </div>
//   `;
// }

// function grantPermission(noteId) {
//   Swal.fire({
//     title: "Grant Permission",
//     text: `Are you sure you want to grant permission for Note ID: ${noteId}?`,
//     icon: "question",
...
//       // Implement logic to grant permission for the record with the given 'noteId'
//       console.log(`Granted permission for Note ID: ${noteId}`);
//       // You can replace this with your custom logic for granting permission
//     }
//   });
// }

// function denyPermission(noteId) {
//   Swal.fire({
//     title: "Deny Permission",
//     text: `Are you sure you want to deny permission for Note ID: ${noteId}?`,
//     icon: "warning",
//     showCancelButton: true,
...
//   }).then((result) => {
//     if (result.isConfirmed) {
//       // Implement logic to deny permission for the record with the given 'noteId'
//       console.log(`Denied permission for Note ID: ${noteId}`);
//       // You can replace this with your custom logic for denying permission
//     }
//   });
// }
//search input___________________________________
