...
const stateSearch = document.getElementById("searching");
...
const toggleTableContainer = () => {
  ...
  if (tableContainer.style.display === "none") {
    ...
    showTableButton.style.backgroundColor = "rgb(52,58,64)";
...
function uploadFile(Id) {
  ...
  // Make a single fetch call to upload all files with the JWT token
  fetch(`/upload`, {
    method: "POST",
    headers: {
  ...
  fetch(`/checkTable`, {
    ...
    .then((result) => {
      if (result.exists) {
        ...
        storeDataCallback(); // Call the inner function directly
      }
    })
    .catch((error) => {
...
function updateTableData(tableName, rowId, updatedValue, oldValue) {
  console.log("the updateTableData", tableName);
  console.log("the updateTableData", rowId);
  console.log("the updateTableData", updatedValue);
  console.log("the updateTableData", oldValue);
  ...
  try {
    ...
    const response = fetch(`/updateTableCell/${tableName}/${rowId}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    // Check the response for a success message or handle it accordingly
    if (data.message) {
      console.log(data.message); // Log the success message
      displayPopup(`${rowId}: Updated ${tableName}`, "success");
    } else {
      console.error(
        "Error updating table data: No success message found in response"
      );
    }
...
// }
//admin Operations___________________________________
document.getElementById("usersInfo").addEventListener("click", function () {
  try {
    const tableName = "users";
    DisplayTableData(tableName);
  } catch (error) {
    console.error("there is an error displaying the table", error);
  }
});
